...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option(
            ...
            action='store',
            dest='port',
            default=getattr(settings, 'CHAT_SOCKJS_PORT', 9999),
    ...
    def handle(self, **options):
        if len(settings.CHAT_SOCKJS_CLASSES) > 1:
            from django.core.exceptions import ImproperlyConfigured
            raise ImproperlyConfigured(
                "Multiple connections not yet supported"
            )

        module_name, cls_name = settings.CHAT_SOCKJS_CLASSES[0].rsplit('.', 1)
        module = import_module(module_name)
        cls = getattr(module, cls_name)
        channel = getattr(settings, 'CHAT_SOCKJS_CHANNEL', '/echo')
